...
SamRecord::SamRecord(const SamRecord &record)
    : myStatus(record.myStatus), myRefPtr(record.myRefPtr),
      mySequenceTranslation(record.mySequenceTranslation) {
    copy(record);
}

/*
SamRecord &SamRecord::operator=(const SamRecord &record) {
    copy(record);
    return (*this);
}
*/
...
bool SamRecord::copy(const SamRecord &target) {
    // First allocate size
    allocatedSize = target.allocatedSize;

    // Then malloc using allocated size
    myRecordPtr = (bamRecordStruct *)malloc(allocatedSize);

    // Rmb to copy over cigar buffer
    myCigarTempBufferAllocatedSize = target.myCigarTempBufferAllocatedSize;
    memcpy(myCigarTempBuffer, target.myCigarTempBuffer, myCigarTempBufferAllocatedSize);

    // Memcpy the record ptr
    memcpy(myRecordPtr, target.myRecordPtr, target.allocatedSize);
    ...
    memcpy(&(myRecordPtr->myData), target.myRecordPtr->myData,
           myRecordPtr->myReadNameLength);
    // TODO: not sure if it's doing the right thing, I want to copy over all the
    // data
    ...
    myReadName = target.myReadName;
    myReferenceName = target.myReferenceName;
    myMateReferenceName = target.myMateReferenceName;
    myCigar = target.myCigar;
    mySequence = target.mySequence;
    myQuality = target.myQuality;
    myNeedToSetTagsFromBuffer = target.myNeedToSetTagsFromBuffer;
    myNeedToSetTagsInBuffer = target.myNeedToSetTagsInBuffer;

    // Initialize the calculated alignment info to the uncalculated value.
    myAlignmentLength = target.myAlignmentLength;
    myUnclippedStartOffset = target.myUnclippedStartOffset;
    myUnclippedEndOffset = target.myUnclippedEndOffset;

    // Set that the variable length buffer fields are valid.
    myIsReadNameBufferValid = target.myIsReadNameBufferValid;
    myIsCigarBufferValid = target.myIsCigarBufferValid;
    myPackedSequence = target.myPackedSequence;
    myIsSequenceBufferValid = target.myIsSequenceBufferValid;
    myBufferSequenceTranslation = target.myBufferSequenceTranslation;

    myPackedQuality = target.myPackedSequence;
    myIsQualityBufferValid = target.myIsQualityBufferValid;
    myIsTagsBufferValid = target.myIsTagsBufferValid;
    myIsBinValid = target.myIsBinValid;

    myCigarTempBufferLength = target.myCigarTempBufferLength;

    // extras = target.extras; 
    /** @TODO: Need to write copy constructor for this
     */
    strings = target.strings;
    integers = target.integers;
    intType = target.intType;
    floats = target.floats;
    myTagBufferSize = target.myTagBufferSize;

    myStatus = SamStatus::SUCCESS;

    NOT_FOUND_TAG_STRING = target.NOT_FOUND_TAG_STRING;
    NOT_FOUND_TAG_INT = target.NOT_FOUND_TAG_INT; // TODO - deprecate
    return true;
}

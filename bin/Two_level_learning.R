		...
		for (k in 1:argsL$outerCV){
			...
			if(argsL$regularisation=="E"){   
				if(argsL$fixedAlpha==-1){
					if(is.null(argsL$constraint)){
						#Why keep=TRUE changes elasticnet???
				...
				if (argsL$regularisation=="E"){
						if(argsL$fixedAlpha==-1){
								...
								#}
								 ...
								 #ERROR HANDLING
							        constant.y <- tryCatch( A[j]<-min(elasticnet[[j]]$cvm),error=function(e) {print("constant response y in innerCV -> skip sample"); 
																		constant.response <<-TRUE})
								#if(inherits(constant.y, "error")) break
								if(constant.response) {
									validSamples[i]=FALSE
									break;	
								}		
							...
							#Laura: Multiple Minimum Error values possible -> choose first indexa
							...
							if(length(elasticnet[[1]]) > 1){
								if (argsL$regularisation=="E"){
									if(argsL$fixedAlpha==-1){
										for (j in 1:length(alphaslist)) {
											#A[j]<-min(elasticnet[[j]]$cvm)
											constant.y <- tryCatch( A[j]<-min(elasticnet[[j]]$cvm),error=function(e) {print("constant response y in innerCV -> skip sample");constant.response <<-TRUE})
											if(constant.response) {
												validSamples[i]=FALSE
				                                                                break;
											}
